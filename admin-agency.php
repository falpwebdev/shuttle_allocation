...
<div class="container-fluid">
  <div class="row">
    <div class="col-lg-12">
      <div class="card">
          <h3 class="card-title text-center mt-2">Employees of <?=$agency?></h3>
        <div class="card-body">
          <div class="row">
              ...
              <div class="col-lg-6">
                <?php
                  // Generate Count of Employees
                    $sql = "SELECT COUNT(idNumber) AS totA FROM `a_m_employee` WHERE `status` = 'Active' AND empAgency = '$employer'";
                    $query = $conn->query($sql);
                      $data = $query->fetch_assoc();
                      $countA = $data['totA'];
                    $sql = "SELECT COUNT(idNumber) AS totS FROM `a_m_employee` WHERE `status` = 'Suspended' AND empAgency = '$employer'";
                    $query = $conn->query($sql);
                      $data = $query->fetch_assoc();
                      $countS = $data['totS'];
                      $total = $countA + $countS;
...
$(document).ready(function(){
  $('#navEmp').addClass('font-weight-bold');
  $('#navEmp').css('color','#CC0000');
});
  //  FUNCTIONS TO UI
    // TABLE
      // Display Section of Selected 
      ...
      // ADD
        ...
        // Display Position
          const displayPos = () => {
            $.ajax({
              url: 'functions/display/common_department.php?data=ma_position',
              method: 'get',
              success: function(response){
  ...
  // Show Table
      ...
      $('#btnShowEmp').click(function(){
          ...
          if(section != '0' && $('#selectDept').is(":disabled") && $('#empIdNum').is(":disabled")){
            ...
            $.ajax({
              url: 'functions/display/admin_emp_master.php?data=master_dept&type='+type+'&data1='+data+'&agency=<?=$employer?>',
              method: 'get',
              success: function(response){
      ...
      $('#submitNewEmp').click(function(){
        ...
        $(".modal-body select").each(function() {
          ...
          if(stat1 == 'complete' && stat2 == 'complete'){
            ...
            if(document.getElementById('checkTempo').checked){
              var jobType = 'Temporary';
            }else{
