    ...
    if ($fileType == "xls" || $fileType == "xlsx") {
      ...
      if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetDir . $newFileName)){
        // Read Excel File
          ...
          $reader->setLoadSheetsOnly('Update Date Hired');
          ...
          for ($i=0; $i < $highestRow; $i++) { 
            ...
            // Check if Exist in Master
              if($idNumber != ''){
                ...
                if($count != 0){
                  // COST CENTER
                    $dateHired = $worksheet->getCell('B'.$startRow)->getValue();
                  // Update Employee Cost Center
                    $sql = "UPDATE `a_m_employee` SET `dateHired`='$dateHired' WHERE `idNumber` = '$idNumber'";
                    $query = $conn->query($sql);
                    if($query){
                      // Record Emp History
                      $sqlInsertRec = "INSERT INTO `a_mp_history`(`idNumber`, `activityDate`, `actDescription`, `user`) VALUES ('$idNumber',(SELECT CURRENT_TIMESTAMP()),'Update Date Hired via Manpower Adjustment','Basic User')";
        ...
        // Record User Logs
          $sqlInsertLogs = "INSERT INTO `sas_logs`(`activityDate`, `userID`, `userName`, `actDescription`, `ipAdd`) VALUES ((SELECT CURRENT_TIMESTAMP()),'Basic User','Basic User','Update Date Hired via Manpower Adjustment','$ipAdd')";
